...
class EndEffectorMarkerNode(Node):
    def __init__(self):
        ...
        self.declare_parameter('eq_pose_topic_name', 'cartesian_impedance/equilibrium_pose')
        self.declare_parameter('gripper_topic_name', 'panda_gripper/gripper_command')
        self.declare_parameter('base_link', 'panda_link0')
        self.declare_parameter('ee_link', 'panda_hand_tcp')

        self.eq_pose_topic_name = self.get_parameter('eq_pose_topic_name').get_parameter_value().string_value
        ...
        self.get_logger().info(f"Publishing eq. pose to topic: {self.eq_pose_topic_name}")
        self.get_logger().info(f"Publishing gripper cmd to topic: {self.gripper_topic_name}")
    ...
    def handle_menu_feedback(self, feedback: InteractiveMarkerFeedback):
        ...
        if feedback.menu_entry_id == self.grasp_entry:       
            ...
            self.grasp_msg.header.stamp = self.get_clock().now().to_msg()
        ...
        if feedback.menu_entry_id == self.open_entry: 
            ...
            self.open_msg.header.stamp = self.get_clock().now().to_msg()
            self.gripper_pub.publish(self.open_msg)
